...
void addPoint(int x,int z,int y){
   ...
   // hashmap_put(usedmap,idx,NULL);
    PNode* point=malloc(sizeof(PNode));
    point->x=x;
    point->y=y;
    point->z=z;
    point->next=plist;
...
void addPoint2(int x,int z,int y,int type,int level){
    ...
    PNode2* point=malloc(sizeof(PNode2));
    point->x=x;
    point->y=y;
    point->z=z;
    point->type=type;
...
inline int genLevel(int type,int level){
    if(type==TYPE_WATER){
...
inline int getBaseType(int type){    
