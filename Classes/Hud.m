...
- (void)renderBlockAndBorder:(CGRect) recto{
    ...
    CGPoint tp;
    if(type==TYPE_TNT||type==TYPE_FIREWORK||type==TYPE_LADDER){
        if(type==TYPE_TNT){
...
- (void)renderBlockScreen{	
	...
	for(int i=0;i<NUM_DISPLAY_BLOCKS;i++){
		...
		CGPoint tp;
		if(type==TYPE_TNT||type==TYPE_LADDER||type==TYPE_FIREWORK){
            if(type==TYPE_TNT)
                tp=[[Resources getResources] getBlockTex:TEX_TNT_SIDE_COLOR];
            else if(type==TYPE_FIREWORK){
        ...
        GLfloat				vertices2[] = {
		 ...
		 if(type==TYPE_FIREWORK||type==TYPE_GRASS2||type==TYPE_GRASS3||type==TYPE_TNT||type==TYPE_BRICK||type==TYPE_VINE)
