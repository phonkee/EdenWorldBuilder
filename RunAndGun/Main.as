package {
	...
	public class Main {
		...
		function updateAndCollideBullets() {

			var bl: Particle;
			var m: DisplayObject;
			var bullet_speed: Number = 15;
			for (var i = 0; i < bullets.length; i++) {
				...
				if (bl != null) {
					...
					if (i != -1) {
						var ncol = lvl1.collidable.numChildren;

						/*var circle2: Shape = new Shape();
						circle2.graphics.beginFill(0x0000FF);
						circle2.graphics.drawCircle(0,0, 3);
						circle2.x = bl.x+bl.point2.x;
						circle2.y = bl.y+bl.point2.y;
						lvl1.addChild(circle2);*/


						var remove = 0;
						var bounds_obj: Rectangle = lvl1.getBounds(lvl1.parent);
						if (bl.y < bounds_obj.top) {
							remove = 1;
						}
						if (bl.y > bounds_obj.bottom) {
							remove = 1;
						}
						if (remove == 0)
							for (var colidx = 0; colidx < ncol; colidx++) {
								m = DisplayObject(lvl1.collidable.getChildAt(colidx));
								//if (m.name == "instance4") continue;
								if (m is Particle) continue;

								if (bl.hitTestObject(m)) {
								
									if (m.hitTestPoint(bl.x+lvl1.x+bl.point2.x, bl.y+lvl1.y+bl.point2.y, true)) {
											...
											lvl1.addChild(circle2);
											trace(m.name);
											if(m==lvl1.collidable.enemy1){
												lvl1.collidable.enemy1.rotation+=10;
											}
									remove=1;
											break;
										}
			...
			function fl_EnterFrameHandler(event: Event): void {
				...
				if (mdown) {
					...
					if (mdown1 || gun_delay == 10) {
						...
						b.graphics.lineTo(p2.x - p1.x, p2.y - p1.y)
						//b.graphics.lineTo(p2.x, p2.y);
						b.lvlp = new Point(ball1.x, ball1.y);

						b.point2 = new Point(p2.x- p1.x, p2.y- p1.y);
						b.x = p1.x;
						b.y = p1.y;
						bullets.push(b);
